  ...
  it("D/T/M: ZIP stays, unconfigured GitHub is honest, no auto export, no overflow", async () => {
    ...
    try {
      ...
      for (const vp of viewports) {
        ...
        page.on("request", (req) => {
          if (req.method() === "POST" && /\/api\/github\/export/.test(req.url())) posts.push(req.url());
        });
        await seed(page, ready(pid, now), owner);
        await page.goto(`${PREVIEW}/studio/${pid}`, { waitUntil: "domcontentloaded", timeout: 20000 });
        ...
        await dialog.getByRole("status").filter({ hasText: /GitHub non configurato/i }).waitFor({ timeout: 8000 });
    ...
    try {
      ...
      await page.route("**/api/github", async (route) => {
        ...
        if (req.method() === "POST") {
          await route.fulfill({
            status: 200,
            contentType: "application/json",
            body: JSON.stringify({ url: "https://github.com/apps/fenix-export/installations/new?state=test" }),
          });
          return;
        }
      ...
      await page.route("**/api/github/repos", async (route) => {
        await route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            repos: [{ fullName: "krelunaid/argilla", defaultBranch: "main", private: false, empty: false }],
          }),
        });
      });
      ...
      const selectH = await dialog.locator("select").evaluate((el) => el.getBoundingClientRect().height);

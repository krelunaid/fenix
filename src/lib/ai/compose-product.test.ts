...
describe("graphic pipeline prompt→plan→generate→visual→QA", () => {
  ...
  it("raises field-product chrome toward a real product without Ciao or a house palette", () => {
    ...
    assert.match(product.html, /fx-shell/);
    ...
    assert.match(product.html, /--water:/);
    assert.match(product.html, /#0[Dd]73[Cc]4|#178[Aa]45|#121[Cc]2[Dd]/);
    assert.doesNotMatch(product.html, /Ciao/);
    assert.doesNotMatch(product.html, /#b51246|#0071e3|#f5f5f7|#007aff/i);
    assert.notEqual(product.tokens.palette.accent.toLowerCase(), "#b51246");
    assert.equal(product.tokens.palette.accent.toLowerCase(), "#0d73c4");
    assert.equal(product.tokens.palette.success.toLowerCase(), "#178a45");
    assert.equal(product.tokens.palette.fg.toLowerCase(), "#0a2f6b");

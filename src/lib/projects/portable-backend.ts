...
export const PORTABLE_COLLECTION_SCOPES = ["owner", "org", "catalog"] as const;
export type PortableCollectionScope = (typeof PORTABLE_COLLECTION_SCOPES)[number];

export type PortableBackendCollection = {
  name: string;
  fields: PortableBackendField[];
  /** owner: only own rows. org: admin sees all, members own rows. catalog: all can read, admin writes. */
  scope?: PortableCollectionScope;
};
...
export const PORTABLE_BACKEND_VERSION = 6;
export const PORTABLE_SCHEMA_VERSION = 5;
...
export function validatePortableBackendSpec(spec: PortableBackendSpec): string[] {
  ...
  for (const collection of spec.collections) {
    ...
    if (collection.scope && !PORTABLE_COLLECTION_SCOPES.includes(collection.scope)) {
      errors.push(`Scope non valido in ${collection.name}: ${String(collection.scope)}`);
    }
...
`,
    ...
    {
      id: "0005_roles",
      filename: "0005_roles.sql",
      sql: `ALTER TABLE _fenix_users ADD COLUMN role TEXT NOT NULL DEFAULT 'member';
`,
    },
  ];
}
...
const hasRole = db.prepare("PRAGMA table_info(_fenix_users)").all().some((col) => col.name === "role");
...
function bearerPrincipal(req) {
  ...
  return a.length === b.length && a.length > 0 && timingSafeEqual(a, b)
    ? { id: serviceId, email: "service@fenix.local", service: true, role: "admin" }
    : null;
}
...
function sessionPrincipal(req) {
  ...
  const found = hasRole
    ? db.prepare("SELECT u.id,u.email,u.role FROM _fenix_sessions s JOIN _fenix_users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?").get(sha256(raw), now)
    : db.prepare("SELECT u.id,u.email FROM _fenix_sessions s JOIN _fenix_users u ON u.id=s.user_id WHERE s.token_hash=? AND s.expires_at>?").get(sha256(raw), now);
  return found ? { id: found.id, email: found.email, service: false, role: found.role || "member" } : null;
}

function userPublic(actor) {
  const role = actor.role || (actor.service ? "admin" : "member");
  return { id: actor.id, email: actor.email, service: !!actor.service, role };
}

function lookupRole(userId) {
  if (!hasRole || !userId) return "member";
  const found = db.prepare("SELECT role FROM _fenix_users WHERE id=?").get(userId);
  return found && found.role ? found.role : "member";
}
...
function row(value) {
  ...
  return { id: value.id, owner_id: value.owner_id, ...JSON.parse(value.data), created_at: value.created_at, updated_at: value.updated_at, version: value.version };
}

function collectionScope(collection) {
  const scope = collection && collection.scope;
  return scope === "org" || scope === "catalog" ? scope : "owner";
}

function isAdmin(actor) {
  return !!(actor && (actor.service || actor.role === "admin"));
}

function canReadRow(actor, collection, ownerId) {
  const scope = collectionScope(collection);
  if (scope === "catalog") return true;
  if (scope === "org" && isAdmin(actor)) return true;
  return ownerId === actor.id;
}

function canWriteRow(actor, collection, ownerId) {
  const scope = collectionScope(collection);
  if (scope === "catalog") return isAdmin(actor);
  if (scope === "org" && isAdmin(actor)) return true;
  return ownerId === actor.id;
}

function listOwned(name, actor, collection, limit) {
  const scope = collectionScope(collection);
  if (scope === "catalog" || (scope === "org" && isAdmin(actor))) {
    return db.prepare('SELECT * FROM "' + name + '" ORDER BY updated_at DESC LIMIT ?').all(limit).map(row);
  }
  return db.prepare('SELECT * FROM "' + name + '" WHERE owner_id=? ORDER BY updated_at DESC LIMIT ?').all(actor.id, limit).map(row);
}
  ...
  try {
    ...
    if (url.pathname === "/auth/signup" && req.method === "POST") {
      ...
      try {
        ...
        const humans = db.prepare("SELECT COUNT(*) AS n FROM _fenix_users WHERE id!=?").get(serviceId).n;
        const role = hasRole && Number(humans) === 0 ? "admin" : "member";
        if (hasRole) {
          db.prepare("INSERT INTO _fenix_users (id,email,password_hash,created_at,role) VALUES (?,?,?,?,?)").run(userId, email, passwordRecord(password), new Date().toISOString(), role);
        } else {
        ...
        db.exec("COMMIT");
        return json(res, 201, userPublic({ id: userId, email, service: false, role }), { "set-cookie": sessionCookie(raw) });
      } catch (error) {
        try { db.exec("ROLLBACK"); } catch {}
        throw error;
      }
    ...
    if (url.pathname === "/auth/login" && req.method === "POST") {
      ...
      const password = typeof input.password === "string" ? input.password : "";
      const user = hasRole
        ? db.prepare("SELECT id,email,password_hash,role FROM _fenix_users WHERE email=?").get(email)
        : db.prepare("SELECT id,email,password_hash FROM _fenix_users WHERE email=?").get(email);
      ...
      return json(res, 200, userPublic({ id: user.id, email: user.email, service: false, role: user.role || "member" }), { "set-cookie": sessionCookie(raw) });
    }
    ...
    if (url.pathname === "/auth/passwordless/verify" && req.method === "POST") {
      ...
      if (rawToken) {
        ...
        try {
          ...
          db.exec("COMMIT");
          return json(res, 200, userPublic({ id: row.user_id, email: row.email, service: false, role: lookupRole(row.user_id) }), { "set-cookie": sessionCookie(raw) });
        } catch (error) {
          try { db.exec("ROLLBACK"); } catch {}
          throw error;
        }
      ...
      try {
        ...
        const raw = createSession(row.user_id);
        db.exec("COMMIT");
        return json(res, 200, userPublic({ id: row.user_id, email: user.email, service: false, role: lookupRole(row.user_id) }), { "set-cookie": sessionCookie(raw) });
      } catch (error) {
        try { db.exec("ROLLBACK"); } catch {}
        throw error;
      }
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (req.method === "GET" && !id) {
        ...
        return json(res, 200, { items: listOwned(name, actor, collection, limit) });
      }
      if (req.method === "GET" && id) {
        const found = db.prepare('SELECT * FROM "' + name + '" WHERE id=?').get(id);
        if (!found || !canReadRow(actor, collection, found.owner_id)) return json(res, 404, { error: "Record non trovato" });
        return json(res, 200, row(found));
      }
      if (req.method === "POST" && !id) {
        if (collectionScope(collection) === "catalog" && !isAdmin(actor)) return json(res, 403, { error: "Non autorizzato" });
        const input = await body(req);
      ...
      if (req.method === "PUT" && id) {
        ...
        if (!existing) return json(res, 404, { error: "Record non trovato" });
        if (!canWriteRow(actor, collection, existing.owner_id)) return json(res, 403, { error: "Non autorizzato" });
        const input = await body(req);
        const invalid = validate(input, collection);
        if (invalid) return json(res, 400, { error: invalid });
        const now = new Date().toISOString();
...
export function materializePortableBackend(spec: PortableBackendSpec): ProjectFile[] {
  ...
  const manifest = {
    ...
    collections: spec.collections.map((collection) => ({
      ...
      scope: collection.scope === "org" || collection.scope === "catalog" ? collection.scope : "owner",
...
/** Keep a materialized Node+SQLite tree when a later HTML-only write omits the manifest. */
export function retainPortableBackendFiles(
  existing: ProjectFile[] | undefined,
  incoming: ProjectFile[] | undefined,
): ProjectFile[] {
  const next = incoming ? [...incoming] : [];
  if (next.some((file) => file.path === PORTABLE_BACKEND_MANIFEST)) return next;
  const keep = (existing || []).filter(
    (file) => file.path === PORTABLE_BACKEND_MANIFEST || isGeneratedPortablePath(file.path),
  );
  if (!keep.length) return next;
  const seen = new Set(next.map((file) => file.path));
  return [...keep.filter((file) => !seen.has(file.path)), ...next];
}

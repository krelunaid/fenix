...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "La direzione grafica esplicita del brief sopravvive a tokensFromBrief, composeProduct, repairBuild (mock dichiarato, liveVerified:false, non un controller-edit live) e prepareSrcDoc. Formulazione italiana «stile iPhone» = system. Negazione scoped alla clausola: «Non usare Literata ma system-ui» = system (ultima richiesta positiva); «Non usare Literata» non inverte; «Non voglio tab Home Aggiungi Persona» non stampa chrome semantic dal raw. CSS comune senza vars, font shorthand (body{font:16px Georgia} e h1{font:700 22px/1.2 Georgia} e le quattro spaziature size/line-height incluso 22px/ 1.2) e selettori qualificati (body.app, h1.title) ricevono lo stack senza perdere font-style/weight/size/line-height; p.quote, button non tipizzati e testo utente restano. Tab Home/Aggiungi/Persona e home richiesta non riscritte solo perché comuni; scheletro 4 riquadri+Ultimo/Stato resta gate. PHONE_KIT: geometria default 5, colonne dal numero reale di tab, nessuna quinta tab fuori riga; non un mandato a 4 colonne. Serif da rivista: Literata. Dominio invariato se il brief non chiede. Clone Apple resta il SET #f5f5f7+#0071e3+SF Pro. Follow-up sola icona: repairBuild mock, non dimostra da solo controller-edit reale. D/T/M/320 computed styles, locator.click CREATE+UPDATE+DELETE+reload, tab ON/OFF contrasto ≥4.5. Serif rail ON: accent-ink su fill accent. SYSTEM first-run: empty onesto, motore palette adattivo + history (non un unico #eceff3/#125e57, non SET Apple), un solo titolo, CTA ≥200×44, pocket-list list-style none, niente — · nuovo, niente paper·glacier·anti-clone né record uno/due/tre. Home overview (conteggio/recenti/empty/CTA) distinta da Elenco CRUD e da Persona storage locale; quattro corpi pairwise-distinct. Persona: wipe-ask → wipe-confirm → persist+reload vuoto. Desktop senza min-height 100dvh stretch. Main 320/390 con padding sotto nav. Tab ON SYSTEM meno pesante (accent su mix, non fill pieno). Screenshot primo schermo attende settle reale (#load/#toast hidden, #err visibile = fail), non maschera overlay. Hash è soglia di movimento, non voto. Nessun 9/10. Non premium.",
    evidence:
      "graphic-intent.ts GRAPHIC_INTENT_PARENT_SHA 76414c7 + graphic-intent.test.ts (stile iPhone, Non usare Literata ma system-ui, Non voglio tab Home, font:16px e font:700 22px/1.2 e 22px/ 1.2, body.app, #eceff3, history SYSTEM) + craft-icons.test.ts hasRequestedWorkingHome + repair.recorded.test.ts mock shorthand weight/slash + intent-preservation-browser.test.ts repairBuild mock computed size/weight/lh+slash spacings+CRUD+CTA+pocket-list+4 viste distinte + phone-kit-tabs-browser.test.ts 5 tab senza data-fenix-phone 320/390/T/D",
    ok: true,
  },
  {
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24 visibili D/T/M dopo prepareSrcDoc (overflow visibile, niente clip dei tracciati), palette diverse dal brief e applicate in CSS calcolato (niente sostituzione navy/ciano di palette calde scure esplicite), tipo operativo 17/headline sans per Agenda (Figtree + system-ui/-apple-system in coda, non Newsreader/Georgia né Inter/SF Pro come primario), imagery di dominio e niente hero KPI o tab Home/Elenco. Chrome da tasca: home first-run (non ledger Voci/Limite/Squadra, non 4 riquadri), raggio CTA ≥8 e hit 44px, tabbar blur. Clone Apple resta il SET grigio+#0071e3+SF Pro, non il fallback -apple-system. Identità bottiglia/piatto (slot sulla riga, artOf) indipendente da meet/slice: stesso prodotto in hero e thumb dopo reorder/wear/update/reload. Hero profumi/ristorazione: meet con aspect-ratio 640/420 a larghezza piena. Miniature profumi: viewBox dedicato (data-focus cap+corpo, tutti gli slot, entrambe le varianti) così il flacone è intero e centrato nella card stretta a 320/390/D/T/M, senza toggle globale meet/slice né bande; hero resta 0 0 640 420. Piatti poco distinguibili restano backlog. Niente ::after « · in prova» del generatore. Azioni riga Agenda su una linea (Avanti + icone Modifica/Archivia 44px, nowrap, overflow visibile). Nav e tre azioni Agenda misurati a 320/390px con bounding box e locator.click. Before/after D/T/M su parent bffc58f. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CREATE+UPDATE+DELETE+reload reale, console pulita dopo le interazioni, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate. Il polish LLM del worker resta saltato per quota; parseBuildOutput/gateBuildResult/repairBuild/prepareSrcDoc hanno una prova con fixture worker-shaped e fetch mock iniettato (liveVerified:false, non risposte xAI live).",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA bffc58f + domain-imagery.ts PERFUME_SUBJECT + perfumeThumbViewBox + wrap slice viewBox/data-focus + artOf(item.slot) + compose-product-browser.test.ts paintPerfumeThumbs cap+body margins all slots 320/390/D/T/M both variants + title→bottle after wear/create/update/reload + CREATE+UPDATE+DELETE+reload D/T/M locator.click + overflow after UPDATE + fixtures/graphic/five before/after + fixtures/graphic/five/contact before/after thumbs",
    ok: true,
  },
  {
    id: "agenda-runtime",
    claim:
      "Agenda generata: settimana Lun-Dom allineata al giorno selezionato (prev/next/oggi senza spostare gli ISO salvati); selezione ISO/ARIA/tastiera; stato distinto dall'orario; form di dominio; create/update/delete/advance con esito bridge (niente falso successo, rollback e form integro se reject/timeout); empty-state unico (dati ⇒ zero empty, zero dati ⇒ uno solo, niente kit «Nessun elemento»). Advance dipinge lo stato in UI prima dell'ACK. Persist serializza su stato confermato + log di operazioni: il save manda solo la head, su reject la coda viene ribasata (due advance sulla stessa riga: reject/reject, ok/reject, reject/ok). Edit è una patch dei campi toccati, non un replace della riga: advance-fail/edit-ok non resuscita lo status fallito; advance-ok/edit-fail tiene il confermato ACK. Un ACK non cancella un form non salvato. Il fallback boot 500ms non azzera pendingOps; un load tardivo ribasa senza scartare la coda. Throw sincrono su Fenix.save non lascia persistDepth busy. 0 crediti xAI. Il five-brief grafico ha before/after su parent a930493 e CREATE+UPDATE+DELETE+reload D/T/M con locator.click sui pulsanti; non è prova di 9/10.",
    evidence:
      "compose-product.ts mondayOf/week-nav/payloadForHead enqueueOp edit-patch renderKeepForm finishBoot + color-scheme.ts productOwnsList + compose-product.test.ts + agenda-runtime-browser.test.ts D/T/M empty-state + bridge reject/timeout + two-row delayed ACK + same-row rebase + Delta patch/keep + Bozza viva + late-boot Taglio e piega + fixtures/shots/agenda-runtime",
    ok: true,
  },
  {
    id: "emergent",

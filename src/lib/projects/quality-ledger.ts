...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA a930493 + color-scheme.ts resolvePalette keeps explicit bg/accent + compose-product-browser.test.ts computed palette after prepareSrcDoc + CREATE+UPDATE+DELETE+reload D/T/M + familyChromeCss + craft-icons.ts pentola a orecchie + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",
    claim:
      "Agenda generata: settimana Lun-Dom allineata al giorno selezionato (prev/next/oggi senza spostare gli ISO salvati); selezione ISO/ARIA/tastiera; stato distinto dall'orario; form di dominio; create/update/delete/advance con esito bridge (niente falso successo, rollback e form integro se reject/timeout); empty-state unico (dati ⇒ zero empty, zero dati ⇒ uno solo, niente kit «Nessun elemento»). Advance dipinge lo stato in UI prima dell'ACK, serializza persist (data-fenix-persist) e su reject ripristina solo quella riga (niente snapshot intero che cancella avanzamenti in coda). Throw sincrono su Fenix.save non lascia persistDepth busy. 0 crediti xAI. Residui fonte, non dichiarati runtime-proven: due advance sulla stessa riga con fail del secondo dopo successo del primo non è coperto dall'E2E a due righe. Il five-brief grafico ha before/after su parent a930493 e CREATE+UPDATE+DELETE+reload D/T/M; non è prova di 9/10.",
    evidence:
      "compose-product.ts mondayOf/week-nav/persistThen surgical revert + color-scheme.ts productOwnsList + compose-product.test.ts + agenda-runtime-browser.test.ts D/T/M empty-state + bridge reject/timeout + two-row delayed ACK queue rebase + fixtures/shots/agenda-runtime",
    ok: true,
  },
  {
    id: "emergent",

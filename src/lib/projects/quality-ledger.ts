...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24, palette diverse dal brief e applicate dopo prepareSrcDoc (niente sostituzione navy/ciano di palette calde scure esplicite), tipo operativo 17/headline sans per Agenda (Figtree + system-ui, non Newsreader/Georgia né Inter/SF Pro), imagery di dominio e niente hero KPI o tab Home/Elenco. Before/after D/T/M su parent a930493. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CREATE+UPDATE+DELETE+reload reale, console pulita dopo le interazioni, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate.",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA a930493 + color-scheme.ts resolvePalette keeps explicit bg/accent + compose-product-browser.test.ts computed palette after prepareSrcDoc + CREATE+UPDATE+DELETE+reload D/T/M locator.click + overflow after UPDATE + familyChromeCss + craft-icons.ts pentola a orecchie + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",

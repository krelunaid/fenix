...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24 visibili D/T/M dopo prepareSrcDoc (overflow visibile, niente clip dei tracciati), palette diverse dal brief e applicate in CSS calcolato (niente sostituzione navy/ciano di palette calde scure esplicite), tipo operativo 17/headline sans per Agenda (Figtree + system-ui/-apple-system in coda, non Newsreader/Georgia né Inter/SF Pro come primario), imagery di dominio e niente hero KPI o tab Home/Elenco. Chrome da tasca: home first-run (non ledger Voci/Limite/Squadra, non 4 riquadri), raggio CTA ≥8 e hit 44px, tabbar blur. Clone Apple resta il SET grigio+#0071e3+SF Pro, non il fallback -apple-system. Identità bottiglia/piatto (slot sulla riga, artOf) indipendente da meet/slice: stesso prodotto in hero e thumb dopo reorder/wear/update/reload. Hero profumi/ristorazione: meet con aspect-ratio 640/420 a larghezza piena (niente bande laterali da min/max-height), miniature slice, lista in .collection/.tickets con align-items:start (la card non si stira sulla riga dell'hero), didascalia statica sotto l'SVG. Niente ::after « · in prova» del generatore. Azioni riga Agenda su una linea (Avanti + icone Modifica/Archivia 44px, nowrap, overflow visibile). Nav e tre azioni Agenda misurati a 320/390px con bounding box e locator.click. Before/after D/T/M su parent 9bb9a3c. Miniature bottiglia/piatto ancora tagliate restano backlog. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CREATE+UPDATE+DELETE+reload reale, console pulita dopo le interazioni, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate. Il polish LLM del worker resta saltato per quota; parseBuildOutput/gateBuildResult/repairBuild/prepareSrcDoc hanno una prova con fixture worker-shaped e fetch mock iniettato (liveVerified:false, non risposte xAI live).",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA 9bb9a3c + PHONE_KIT radius 12/system stack/tabbar blur + craft-icons.ts FIRST_RUN_HOME + graphic-quality.ts CLONE_FACE SF only + domain-imagery wrap fit arg + gid per meet/slice + artOf(item.slot) + .collection align-items:start + hero aspect-ratio 640/420 + caption static + color-scheme.ts resolvePalette keeps explicit bg/accent + compose-product-browser.test.ts computed palette/icons after prepareSrcDoc + DOM cardH vs hero + title→bottle/dish after wear + CREATE+UPDATE+DELETE+reload D/T/M locator.click + overflow after UPDATE + nav/agenda hit-boxes 320/390 + nav icons overflow:visible D/T/M + slot-actions nowrap overflow visible + familyChromeCss + craft-icons.ts pentola a orecchie overflow visible + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + preview-contrast pocket quality + repair.recorded.test.ts parseBuildOutput/gateBuildResult/repairBuild/prepareSrcDoc liveVerified false fixtures/recorded + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",

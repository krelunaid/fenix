...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24, palette diverse dal brief e applicate (override utente conservato), tipo operativo 17/headline sans per Agenda (Figtree + system-ui, non Newsreader/Georgia né Inter/SF Pro), imagery di dominio e niente hero KPI o tab Home/Elenco. Before/after D/T/M su parent 6a3dd1c. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CRUD reale, console pulita, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate.",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA 6a3dd1c + isOperationalApp + familyChromeCss + craft-icons.ts pentola a orecchie + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + compose-product-browser.test.ts D/T/M CRUD + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",
    claim:
      "Agenda generata: settimana Lun-Dom allineata al giorno selezionato (prev/next/oggi senza spostare gli ISO salvati); selezione ISO/ARIA/tastiera; stato distinto dall'orario; form di dominio; create/update/delete/advance con esito bridge (niente falso successo, rollback e form integro se reject/timeout); empty-state unico (dati ⇒ zero empty, zero dati ⇒ uno solo, niente kit «Nessun elemento»). 0 crediti xAI. Residui fonte, non dichiarati runtime-proven: saveOnce avvolge Fenix.save in Promise.resolve e non cattura un throw sincrono prima della Promise; bridge assente risolve ok; reject/timeout solo su M e saves>=2 non prova il limite esatto. settleVisual #toast 4s vs ping 1600ms: un fail isolato con suite da ~15 min non è riprodotto (hide a ~1.8s). Timeout caffeinate su 77 (click save, boot-ok) coincidono con Maintenance Sleep ~900s: non prova causale del prodotto, né due suite verdi. Il five-brief grafico ha before/after su parent 6a3dd1c; non è prova di 9/10.",
    evidence:

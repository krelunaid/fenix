...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24 visibili D/T/M dopo prepareSrcDoc (overflow visibile, niente clip dei tracciati), palette diverse dal brief e applicate in CSS calcolato (niente sostituzione navy/ciano di palette calde scure esplicite), tipo operativo 17/headline sans per Agenda (Figtree + system-ui, non Newsreader/Georgia né Inter/SF Pro), imagery di dominio e niente hero KPI o tab Home/Elenco. Azioni riga Agenda su una linea (Avanti/Modifica/Archivia, nowrap). Before/after D/T/M su parent 8d98b42. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CREATE+UPDATE+DELETE+reload reale, console pulita dopo le interazioni, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate. Il polish LLM del worker resta saltato per quota.",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA 8d98b42 + color-scheme.ts resolvePalette keeps explicit bg/accent + compose-product-browser.test.ts computed palette/icons after prepareSrcDoc + CREATE+UPDATE+DELETE+reload D/T/M locator.click + overflow after UPDATE + nav icons overflow:visible D/T/M + slot-actions nowrap + familyChromeCss + craft-icons.ts pentola a orecchie overflow visible + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",

...
export const QUALITY_LEDGER: LedgerRow[] = [
  ...
  },
  {
    ...
    claim:
      "Il generatore condiviso (composeProduct, non le sole fixture) produce cinque brief distinti — Agenda, profumi, abbigliamento, note/repository, ristorazione — con icone semantiche su griglia 24 visibili D/T/M dopo prepareSrcDoc (overflow visibile, niente clip dei tracciati), palette diverse dal brief e applicate in CSS calcolato (niente sostituzione navy/ciano di palette calde scure esplicite), tipo operativo 17/headline sans per Agenda (Figtree + system-ui, non Newsreader/Georgia né Inter/SF Pro), imagery di dominio e niente hero KPI o tab Home/Elenco. Hero profumi/ristorazione: meet (non slice) e spazio contenuto; niente ::after « · in prova» del generatore. Azioni riga Agenda su una linea (Avanti + icone Modifica/Archivia 44px, nowrap, overflow visibile). Nav e tre azioni Agenda misurati a 320/390px con bounding box e locator.click. Before/after D/T/M su parent 4f7235e. Hash e ΔE sono soglie di movimento, non voti. D/T/M, CREATE+UPDATE+DELETE+reload reale, console pulita dopo le interazioni, AA, overflow/clipping chiusi. Nessun 9/10 senza prova. 0 crediti xAI su questi gate. Il polish LLM del worker resta saltato per quota; parse/gate hanno una prova con risposte registrate e mock dichiarati (liveVerified:false).",
    evidence:
      "compose-product.ts GRAPHIC_FIVE_PARENT_SHA 4f7235e + domain-imagery wrap meet perfume/food + color-scheme.ts resolvePalette keeps explicit bg/accent + compose-product-browser.test.ts computed palette/icons after prepareSrcDoc + CREATE+UPDATE+DELETE+reload D/T/M locator.click + overflow after UPDATE + nav/agenda hit-boxes 320/390 + nav icons overflow:visible D/T/M + slot-actions nowrap overflow visible + familyChromeCss + craft-icons.ts pentola a orecchie overflow visible + design-tokens booking Figtree 17/2.125 + compose-product.test.ts five briefs + repair.recorded.test.ts liveVerified false + fixtures/graphic/five before/after",
    ok: true,
  },
  {
    id: "agenda-runtime",

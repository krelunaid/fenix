...
export function craftNavIcon(tab: { id: string; label: string }, index = 0): string {
  ...
  else if (/^elenco$|^lista$|^libri$|^biblioteca$|^catalogo$/.test(label)) svg = NAV_ICONS.book;
  else if (/^prestiti$|^prestito$/.test(label)) svg = NAV_ICONS.envelope;
  else if (/^scaffali$|^scaffale$/.test(label)) svg = NAV_ICONS.crate;
  else if (/^scheda$/.test(label)) svg = NAV_ICONS.person;

...
export const FASE3_GAPS: Fase3Gap[] = [
  ...
  },
  {
    ...
    fenix:
      "Fenix.load/save e Fenix.data JSON usano sulle app pubblicate un archivio Postgres cloud-private con cookie HttpOnly, isolamento, corpo bounded e CAS; link viewer/editor revocabili condividono il dataset cross-device. In più, i brief full-stack espliciti (login, ruoli, database) esportano un backend Node+SQLite accoppiato al frontend sulla stessa origine, con signup/login email-password scrypt, sessioni opache hash-only HttpOnly, ruoli admin/member (primo account = amministratore), scope collezione owner/org/catalog, recupero password enumeration-safe e accesso passwordless magic-link/OTP (token e OTP solo hash, one-shot, TTL bounded, outbox SQLite senza SMTP), Bearer server-to-server, allowlist Origin, CRUD schema-validato, If-Match CAS, burst concorrente reale e migrazioni v1→v5 senza perdita dati. Nessun segreto viene generato. Mancano OAuth/OIDC social, ACL per-record arbitrari, database distribuito generato e connettori applicativi a pagamento.",
    impact: "high",
    cost: "high",
    slice: "next",
  },

...
- Auth: scrypt, cookie HttpOnly SameSite=Strict, isolamento per `owner_id`, Bearer opzionale, corpo 256 KB, If-Match CAS. Recupero password enumeration-safe (`POST /auth/recover` sempre `{ ok: true }`, token one-shot hash-only TTL 15 minuti in `_fenix_password_resets`, conferma `POST /auth/reset` con scrypt e revoca sessioni). Accesso passwordless enumeration-safe (`POST /auth/passwordless` magic-link o OTP, conservati solo hash, TTL 10 minuti, one-shot, revoca dei precedenti; `POST /auth/passwordless/verify` crea sessione opaca HttpOnly e invalida le challenge). L'outbox SQLite è il provider email di default; niente SMTP, niente token in JSON/log/localStorage.

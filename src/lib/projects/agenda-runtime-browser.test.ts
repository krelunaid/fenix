...
it("Barber identity survives polish parsing across D/T/M/320 without the rejected raspberry draft", async () => {
  ...
  try {
    for(const [id,viewport] of [...VIEWPORTS,["S",{width:320,height:568}] as const]) {
      ...
      const mark=frame.locator('header [data-fenix-id="icon:app"], .fx-app-mark');
      ...
      assert.match(accent.toLowerCase(),/#d2bfa6|#c4a882/);
      const time=frame.locator(".salon-next .time, .slot .time").first();
      ...
      assert.deepEqual(await frame.locator("nav#tabs button span").allInnerTexts(),["Home","Prenota","I miei"]);

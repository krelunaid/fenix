...
- Arcade acceptance exposed a real substring bug: `app di videogiochi` was classified as `app di video` and wrongly rejected for lacking a clip stage. Client and worker now require a word boundary, with positive video-feed and negative arcade regression assertions.

...
- Authentication never stores or compares plaintext passwords. Use only built-in node:crypto: randomBytes salt + scryptSync hash stored as password_hash (for example \`salt:hexhash\`), and timingSafeEqual for verification. SQL schemas and queries must never contain a standalone \`password\` column; request JSON may contain \`password\`, but persist only its derived hash. Demo seed users follow the same rule.

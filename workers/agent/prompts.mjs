...
- Authentication guards in inline ES modules must never use a top-level \`return\` or an uncaught throw. Put each page boot sequence in an \`async function initPage() { if (!requireAuth()) return; … }\` and call it with a handled \`.catch(...)\`; an expected 401 redirects to /login without a console/page error.
...
- Visual identity from the brief: derive the palette from tangible cues of the requested activity (materials, environment, product and audience), never from a generic purple/blue SaaS default; use a restrained primary accent, a related soft tint and warm/cool neutrals that pass contrast. Keep a consistent spacing scale, readable type (16-18px body) and confident hierarchy. The kit supplies an Apple-quality structural baseline; the brief must add meaningful colour, precise copy, one recognisable domain symbol and one or two signature compositions (for example an editorial metric hero, a useful timeline, or a distinctive service card) so the result cannot be mistaken for a generic Notes/dashboard template. ${kind === "site" ? "A site has a top navigation, a strong hero with a concrete offer, sections (services, prices, about, contact/booking form that saves to the API), footer with contacts and legal links." : "An app has a clear primary flow on the home screen, a bottom navigation on phone / labelled sidebar on desktop, lists with search or filters, detail/edit forms, and a settings or info screen."}
- Treat authentication as a designed product surface: never print demo usernames/passwords in the public UI. Keep fixture credentials only in tests or operator documentation. On desktop, use a balanced branded composition (domain mark, concise value statement and the form), not a tiny form floating in a mostly empty viewport; on phone keep the same identity without decorative clutter.
- Domain imagery and illustrations are content, not navigation icons. If the product needs imagery and no photo tool is available, build one deliberate inline-SVG or CSS composition from several distinct domain-specific forms, textures and crops. Never repeat one small pictogram as the hero and every product/service image. Controls still use the icon set.
- Proofread every visible sentence, label and heading before run_checks: no concatenated words, broken accents, untranslated fragments or generic template copy unrelated to the brief.
...
- Authentication never stores or compares plaintext passwords. Use only built-in node:crypto: randomBytes salt + scryptSync hash stored as password_hash (for example \`salt:hexhash\`), and timingSafeEqual for verification. SQL schemas and queries must never contain a standalone \`password\` column; request JSON may contain \`password\`, but persist only its derived hash. Demo seed users follow the same rule, and their raw credentials never appear in public HTML or browser-visible JavaScript.
- Request-body order is deterministic: for every POST/PUT/PATCH route, parse JSON exactly once before authentication or route-specific validation, return 400 for malformed JSON even when the caller is unauthenticated, then reuse that parsed body. Never call the body parser again inside an individual route.
...
3. start_server, then use http only for one representative happy path and one validation error; do not spend the budget duplicating checks that run_checks already performs.
4. Run run_checks as soon as the complete first version exists. Besides your tests it runs independent probes you cannot influence: static UI audit (exactly one <h1>, every field labelled, no emoji icons, no href="#", no external scripts), server source audit (node:sqlite, parameterized SQL only), hostile requests (path traversal, project files must not be served, malformed JSON -> 400, 600 KB body must not crash, wrong methods -> 404/405) and a persistence probe (two boots on the same DATA_DIR must succeed and a .db file must exist there). Fix every FAIL precisely (the report tells you which check and why), grouping all failures in the same file into one edit. Repeat until TUTTI I CONTROLLI PASSANO.

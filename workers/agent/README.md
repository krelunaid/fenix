...
## Fenix UI kit (added 2026-09-08)

`workers/agent/runtime/fenix-ui.css` is the base stylesheet of every generated app (tokens, reset, type scale, app skeleton with bottom tab bar that becomes a sidebar on desktop, site skeleton with nav/hero/bands/footer, cards, lists, KPIs, buttons, forms, badges, empty states, toasts, dialogs, tables; no external resources, 48 px controls, 16 px inputs, safe areas, focus-visible, reduced motion, optional `data-theme="dark"`). `ui-kit.mjs` seeds it into the sandbox as `public/fenix-ui.css` before the model starts (create and edit), the tools refuse to modify or delete it, the system prompt carries a cheat-sheet of the classes, and the gate check `ui:kit` requires the file intact, linked first in every page and actually used. `styles.css` sets `--fx-*` tokens for the brief and domain rules only. The golden fixture uses the kit; `scripts/agent-ui-kit.test.mjs` covers seeding, guards, the gate and the loop.

## Project memory for edits (added 2026-09-08)

An edit (`instruction` + `files`) may carry `parentJobId`. When the parent job is known to this host (memory or `AGENT_DATA_DIR/jobs`), `projectContext` in `server.mjs` chains its memory: original brief, every previous instruction with the summary the agent gave, and the last summary; the chain is stored in the new job's `input.context`, so it survives across many edits (last 10 instructions, clipped). When the parent is unknown (expired, other host) the Studio's own `context` (kept per browser in `fenix.agent.jobs`) is used instead. `editBrief` renders it as BRIEF ORIGINALE / MODIFICHE PRECEDENTI / STATO ATTUALE and the agent also gets the manifest summary (pages, API) so "rimetti il campo come prima" resolves without re-reading everything. `scripts/agent-memory.test.mjs` covers the chain.

#!/usr/bin/env node
/**
 * Deterministic NordAcqua proofs for apple-craft-v4.
...
const outDir = join(root, "docs/checkpoints/2026-09-05/apple-craft-v4");
const artDir = "/opt/cursor/artifacts/apple-craft-v4";
...
let html = prepareSrcDoc(product.html, product.tokens.palette, "nordacqua-v4", "app");
...
const mark = glossyWaterMarkSvg("nordacqua-v4", product.tokens.palette);
const iconPage = `<!DOCTYPE html><html lang="it"><head><meta charset="utf-8"/><style>
html,body{margin:0;height:100%;background:#F8FAFC;display:grid;place-items:center}
.card{display:grid;place-items:center;gap:14px}
svg{width:168px;height:168px;filter:drop-shadow(0 18px 28px rgba(15,23,42,.22))}
p{margin:0;font:700 22px/1.1 ui-sans-serif,system-ui,sans-serif;color:#0F172A;letter-spacing:-.03em}

...
test("automatic composed polish rejects unsafe, absent and ineffective style plans", { timeout: 30000 }, async () => {
  ...
  try {
    ...
    for (const mode of ['valid', 'unsafe', 'absent', 'ineffective']) {
      ...
      const response = await fetch(`${base}/polish`, {
        ...
        body: JSON.stringify({ prompt: mode === 'unsafe' ? "STYLE_REJECT_FIXTURE" : "Agenda leggibile", html: artifact, kind: "app" }),
      ...
      assert.equal(job.status, mode === 'valid' ? 'ok' : 'err', mode);
      if (mode !== 'valid') {
    ...
    assert.equal((output.match(/VISUAL_STYLE_PROVIDER_CALL/g) || []).length, 4, "one call per request, no icon/image/rewrite fallback");
  } finally {
    worker.kill("SIGTERM");
    await exited;
  }
